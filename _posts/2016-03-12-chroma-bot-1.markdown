...
title:  Episode 02. Chroma Bot Landing
image:  /media/posts/2016-03-12-chroma-bot-1/header.jpg
description: >
  How to build simple web application in Clojure and deploy it to free Heroku
---

Hello, dear friends!

Welcome to Web Fandango series, where we build simple and fun projects in Clojure. Our next project will be very interesting, but kind of simple to implement. It will also be a solid ground for more complex ones that we will be covering later.

I'm going to show you how to build a chat-bot for Telegram. Bots are getting a lot of traction recently and Telegram is a popular messaging platform with very nice API to integrate with. Our bot will be relatively simple, it will wait for a color in a her form like `#dc322f` and send back a picture filled with that color.
...
Let's sketch a list of tools that we are going to need to build it.

### 1. Landing

First, we are going to need a landing page for our bot, just a simple page that will show some information and a welcoming message with a button to add this bot to start a conversation.

### 2. Telegram API

Then we will need our app to connect to Telegram and process everything that users send to it. There is two ways of doning so, as it is said in the [official documentation](https://core.telegram.org/bots/api#getting-updates):

* we provide a public HTTP endpoint, that Telegram will call and feed updates to. This is great for a remotely deployed application, but can be tricky for local development, because people usually don't have public address for their machine and sit behind a router.

* we perform long-polling of Telegram server. This is a continuous calling and waiting on the HTTP endpoint to receive information about messages sent to our bot and then making same call again. This works perfectly for debugging and testing from a local machine.
...
Finally we need some tools to deploy run our app on a remote server, write it's progress to server log and monitor health of our app.

We are going to deploy our bot to Heroku first, because it is dead easy to do and is free for small projects. The only problem with it is that sometimes our bot will die, because of the way Heroku manages resources. In later episodes I will show you how to deploy almost that easy, but with Docker and DigitalOcean.

## Start a new project

Now create a new project using a follwoing command:

    $ lein new compojure chroma-bot

This tells Leiningen to create a fresh project with the name `chroma-bot` from a template called `compojure`. **Compojure** is a wonderful library for building HTTP APIs, that reflects very well structure of your endpoints. You can think of it as a routing layer of our application. It also provides a convenient template to start a new project for Leiningen, which we just used.

I suggest you to start a git repository right after you created a project and add first initial commit at this time, but that is totally up to you.

This template lays a very good file structure for you:
...
* `README.md` is a file that will be describe that project and will be displayed at its GitHub repo. I'm leaving this completely up to you.
* `resources` is for static resources of your application, like css styles for webpages or image assets
* `src` hold your sources either in Clojure or ClojureScript
* `test` is, like you might guessed, for unit tests

Check out `handler.clj` file:
...
That should give you a brief understanding of how namespaces are used. If you want to have a deeper understanding, try [this chapter](http://www.braveclojure.com/organization/) of Clojure for the Brave and True.

Now what about the code?

* `defroutes` defines routes ;) Seriously, it creates a structure that describes how to react for certain endpoints and gives it a name `app-routes`. There are two endpoints inside, *Hello world* response for root and *Not found* for everything else. This is really basic stuff, but look how elegant it looks!
...
Not that much different, right? Same stuff about requiring external modules and a little bit less consise declaration of routes.
...
Amazing thing to note is that project definition this is plain Clojure code and everything there is just keywords, lists, symbols and strings.

`defproject` tells that this expression defines project with name `chroma-bot` and version `0.1.0-SNAPSHOT`. First couple of arguments is project name and version followed by a set of key-value pairs that describes project metadata.
...
* [`ring`](https://github.com/ring-clojure/ring) is an HTTP abstraction for clojure. As documentation says, it is *inspired by Python's WSGI and Ruby's Rack*. There is server that runs our routes and middleware that does jobs like logging errors or converting request to various formats depending on the content-type header.
* [`hiccup`](https://github.com/weavejester/hiccup) is a library for HTML generation straight from the code, which we'll use for landing.
* [`core.async`](https://github.com/clojure/core.async) is a concurrency abstraction, based on idea of channels, also known as [CSP](https://en.wikipedia.org/wiki/Communicating_sequential_processes). It will help us to talk to Telegram API from a local machine using long-polling.
* [`timbre`](https://github.com/ptaoussanis/timbre) is a nice logging library, that beautifully renders exception and works great for both server and client-side code. It will add some beauty to our printing og technical info to console.
* [`omniconf`](https://github.com/grammarly/omniconf) is an amazing library be Grammarly to manage configuration of our application. It is capable of defining parameters, validating them for completeness, reading from file or environment. I encourage you to follow [12 factor](http://12factor.net/) principles and not store any configuration in your code.
...
This should open a http://localhost:8080/ page in your browser wich will look like this:

<!-- this is my retina screenshot, so we force width here -->
<img src="/media/posts/2016-03-12-chroma-bot-1/start.png" width="711px"/>

You can replace "Hello World" in your code, refresh your page in the browser and observe changes immediately. Funcy, huh!

But our page doesn't look that fancy. Let's add some style to it. I hope you are familiar with CSS & HTML :) If not, there is [beautiful and easy tutorial](http://marksheet.io/) that will help you to get deep understanding very quickly.
...
This is some color for our banner. Other css classes are used from superb [Semantic UI](semantic-ui.com) project which provides excellent tools for laying out UI for web application. We are using just a tiny portion of it now, but later it will shine in its full potential when we will be covering more complex UIs in ClojureScript.
...
This might be offbeat for **erb** or **mustache** people, but should be very familiar to **haml** folks. Here we are using [Hiccup](https://github.com/weavejester/hiccup) library for generating HTML. It has dead simple syntax, which you probably already figured our: `[tag.class {:attribute value} children]`.

With it's help we defined `page` function that covers content with meta information. After that we defined home page with some text and a banner to make our landing look pretty.

Refresh our page in your browser and you should see something like this:
...
* `(:gen-class)` tells Leiningen to generate *Java class* from his namespace, which could be used as an entry point for the packaged application. Java class is required, so we can define it as an entry point for `.jar` file.

* `cfg/define` describes configuration structure for our application. We start with only one parameter &mdash; port on which Heroku will run our app. This value will be provided to us through environment variable `PORT` which `omniconf` will pull automatically, based on `:port` name that we gave it as a key.

* `-main` is the conventional name for a method that will be called when the Java class will be called from command line. In that method we call `cfg/populate-from-env` function to pull configuration from environment variables, then validate it with `(cfg/verify :quit-on-error true)`. After that we log simple message that our server has started and, well, start a HTTP server, providing it our template-generated routes by calling `run-jetty`.
...
Now, commit everything we've created so far and push it Heroku's remote repository:
    ...
    # ...
    #
    # really long log of building an app will be here, but I omit it
